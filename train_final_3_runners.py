#!/usr/bin/env python3
"""
Train final model with all 3 runners (You, Salman, Azeem)
...
def main():
    ...
    # 3. Azeem's Chicago 2023 (marathon_20231008): 28.4% pace degradation
    #    First half: 7.78 min/mile → Second half: 9.98 min/mile
    #
    # Note: Automatic detection in extract_all_marathons.py with detailed
    # comments for methodology documentation in final report
    # ========================================================================
    bonked_race_ids = [
        'marathon_20251012',  # User's Chicago 2025
        'marathon_20231008'   # Salman's Chicago 2023, Azeem's Chicago 2023 (same date)
    ]
    filtered_races = [r for r in all_races if r['race_id'] not in bonked_race_ids]

    print(f"\n{'='*80}")
    print("Filtered out races (bonking detected):")
    print(f"{'='*80}")

    bonked_count = {'my_runner': 0, 'runner_2': 0, 'runner_3': 0}
    for race in all_races:
        if race['race_id'] in bonked_race_ids:
            runner_name = {'my_runner': 'You', 'runner_2': 'Salman', 'runner_3': 'Azeem'}[race['runner_id']]
            time_min = race['actual_time_minutes']
            print(f"  • {runner_name}: {race.get('_race_name', race['race_date'][:10])}")
            print(f"    Time: {int(time_min//60)}:{int(time_min%60):02d}")
            bonked_count[race['runner_id']] += 1
    ...
    print(f"Time range: {y.min()//60:.0f}:{y.min()%60:02.0f} - {y.max()//60:.0f}:{y.max()%60:02.0f}")
    ...
    print(f"  MAE: {best_result['mae']:.2f} minutes ({best_result['mae']//60:.0f}:{int(best_result['mae']%60):02d})")
    print("="*80)
    ...
    # Save
    model_path = 'race_time_model_3_runners.pkl'
    ...
    # Count by runner
    user_count = len([r for r in filtered_races if r['runner_id'] == 'my_runner'])
    salman_count = len([r for r in filtered_races if r['runner_id'] == 'runner_2'])
    azeem_count = len([r for r in filtered_races if r['runner_id'] == 'runner_3'])

    print("\n" + "="*80)
    print("Model Training Complete!")
    print(f"  Dataset: {len(filtered_races)} marathons")
    print(f"    • You: {user_count} marathons")
    print(f"    • Salman: {salman_count} marathons")
    print(f"    • Azeem: {azeem_count} marathons")
    print(f"  Filtered: {len(all_races) - len(filtered_races)} bonked races")
    print(f"    • You: {bonked_count['my_runner']}")
    print(f"    • Salman: {bonked_count['runner_2']}")
    print(f"    • Azeem: {bonked_count['runner_3']}")
